"""
Calibrated OMR Engine for QuickCheck
Uses actual image analysis to calibrate bubble positions
...
class CalibratedOMRProcessor:
    """
    Calibrated OMR processor that learns bubble positions from actual images
    """
    ...
    def __init__(self):
        ...
        # Calibrated positions (will be set during calibration)
        self.bubble_positions = None
        self.is_calibrated = False
        
    def calibrate_from_image(self, image: np.ndarray) -> bool:
        """
        Calibrate bubble positions from a reference image
        """
        try:
            print("Calibrating from reference image...")
            
            # Detect bubbles in the image
            bubble_centers = self.detect_all_bubbles(image)
            if len(bubble_centers) < 200:  # Need at least 50 questions worth
                print(f"Not enough bubbles found for calibration: {len(bubble_centers)}")
                return False
            
            # Build calibrated positions
            self.bubble_positions = self.build_calibrated_positions(bubble_centers, image.shape)
            if len(self.bubble_positions) >= 50:
                self.is_calibrated = True
                print(f"Calibration successful! Found {len(self.bubble_positions)} questions")
                return True
            else:
    ...
    def detect_all_bubbles(self, image: np.ndarray) -> List[Tuple[int, int]]:
        """Detect all bubbles in the image using multiple methods"""
        try:
            ...
            # If still not enough, try HoughCircles
    ...
    def build_calibrated_positions(self, bubble_centers: List[Tuple[int, int]], image_shape: Tuple[int, int]) -> Dict[str, Any]:
        """Build calibrated bubble positions from detected centers"""
        try:
    ...
    def process_omr(self, image: np.ndarray, answer_key: Dict[str, str], 
        ...
        try:
            ...
            if not self.is_calibrated:
                if not self.calibrate_from_image(image):
                    print("Failed to calibrate from image")
                    return None
            
            if not self.bubble_positions:
                print("No calibrated positions available")
                return None
            
            # Detect filled bubbles using calibrated positions
            filled_bubbles = self.detect_filled_bubbles(image, self.bubble_positions, bubble_threshold)
            
            # Evaluate answers
            results = self.evaluate_answers(filled_bubbles, answer_key, subjects)
            
            # Create annotated image
            annotated_image = self.create_annotated_image(image, self.bubble_positions, filled_bubbles, results)
    ...
    def detect_filled_bubbles(self, image: np.ndarray, bubble_positions: Dict[str, Any], 
        ...
        """Detect which bubbles are filled using calibrated positions"""
        try:
            # Convert to grayscale
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if len(image.shape) == 3 else image
            ...
            for question_num, options in bubble_positions.items():
                ...
                for option, (x, y) in options.items():
                    ...
                    bubble_radius = 15  # Fixed radius for consistency
